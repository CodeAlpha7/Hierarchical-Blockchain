...
app.get('/mine', function(req, res) {
	...
	// var lengthCompare = bitcoin.pendingTransactions.length;
	// var arraySlicer = 0;
	const requestPromises = [];
	while(bitcoin.pendingTransactions.length > 1) {
		
		if(bitcoin.pendingTransactions.length > TRANSACTION_THRESHOLD){
			// let endArray = bitcoin.pendingTransactions.length - TRANSACTION_THRESHOLD;
			...
			// lengthCompare = lengthCompare - TRANSACTION_THRESHOLD;
			// arraySlicer = arraySlicer + TRANSACTION_THRESHOLD;
			...
			bitcoin.networkNodes.forEach(networkNodeUrl => {
				...
				// const bridgeNode = {
				// 	uri: 'http://localhost:3006/receive-new-block',
				// 	method: 'POST',
				// 	body: {newBlock: newBlock},
				// 	json: true
				// };
			...
			// 	};
				...
				// const bridgeNode = {
				// 	uri: 'http://localhost:3006/transaction/broadcast',
				// 	method: 'POST',
				// 	body: {
				// 		amount: 12.5,
				// 		sender: "00",
				...
				// return rp(requestOptions, bridgeNode);
			//	return rp(requestOptions);						// --------------> needed
			//})													// --------------> needed
			// .then(data => {
			// 	res.json({
			...
			const nonce = bitcoin.proofOfWork(previousBlockHash, currentBlockData);
			const blockHash = bitcoin.hashBlock(previousBlockHash, currentBlockData, nonce);
			const newBlock = bitcoin.createGenesisBlock(nonce, previousBlockHash, blockHash);
	...
	// 	res.json({
	// 		note: "All trasactioned confirmed! ",
	// 	});
