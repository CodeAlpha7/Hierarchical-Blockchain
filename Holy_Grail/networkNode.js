...
app.post('/mine-pos', function (req, res) {
  ...
  app.post('/multi-mine', function (req, res) {
	var nodes = req.body.nodes;
	const leader = bitcoin.proofOfStake(nodes);
	const requestPromises = [];
	const firstBCOptions = {
		uri: 'http://localhost:300' + leader + '/hierarchical-mine',
		method: 'GET',
		// body: {newBlock: newBlock2},
		json: true
	};
	requestPromises.push(rp(firstBCOptions));

	let secondLeader = bitcoin.proofOfStake(nodes);
	secondLeader = secondLeader + 4;
	const secondBCOptions = {
		uri: 'http://localhost:300' + secondLeader + '/hierarchical-mine',
		method: 'GET',
		// body: {newBlock: newBlock2},
		json: true
	};
	requestPromises.push(rp(secondBCOptions));


	// let thirdLeader = bitcoin.proofOfStake(nodes);
	// thirdLeader = thirdLeader + 8;
	// const thirdBCOptions = {
	// 	uri: 'http://localhost:300' + thirdLeader + '/hierarchical-mine',
	// 	method: 'GET',
	// 	// body: {newBlock: newBlock2},
	// 	json: true
	// };
	// requestPromises.push(rp(thirdBCOptions));


	// let fourthLeader = bitcoin.proofOfStake(nodes);
	// fourthLeader = fourthLeader + 12;
	// const fourthBCOptions = {
	// 	uri: 'http://localhost:300' + fourthLeader + '/hierarchical-mine',
	// 	method: 'GET',
	// 	// body: {newBlock: newBlock2},
	// 	json: true
	// };
	// requestPromises.push(rp(fourthBCOptions));
	...
	.then(data => {
		res.json({
			note: `New blocks mined by node ${leader} & ${secondLeader} successfully`,
		});
	});
  });
  

  app.get('/hierarchical-mine', function (req, res) {
	const requestPromises = [];
	...
	while(!bitcoin.pendingTransactions.length < 1) {
		...
		if(bitcoin.pendingTransactions.length > TRANSACTION_THRESHOLD){
			...
			const bridgeNodeOptions = {
				uri: 'http://localhost:3009/receive-global',
				method: 'POST',
				body: {newBlock: newBlock2},
				json: true
			};
